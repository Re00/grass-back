  ...
  <div class="shoppingCart">
    ...
    <div class="content" v-if="getShoppingCart.length>0">
      <ul>
        ...
        <li class="header">
          <div class="pro-check">
            <el-checkbox v-model="isAllCheck" @change="allCheck($event)">全选</el-checkbox>
          </div>
        ...
        <!-- 购物车列表 -->
        <li class="product-list" v-for="(item,index) in getShoppingCart" :key="item.buyerCart.id">
          <div class="pro-check">
            <el-checkbox :value="item.buyerCart.checkType" @change="checkChange($event,index,item.goods.gid)"></el-checkbox>
          </div>
          <div class="pro-img">
            <router-link :to="{ path: '/goods/details', query: {productID:item.goods.gid} }">
          ...
          <div class="pro-name">
            <router-link
              ...
              :to="{ path: '/goods/details', query: {productID:item.goods.gid} }"
            >{{item.goods.gname}}</router-link>
          </div>
          <div class="pro-price">{{item.goods.gpride}}元</div>
          <div class="pro-num">
            <el-input-number
              size="small"
              :value="item.buyerCart.goodsnum"
              @change="handleChange($event,index,item.goods.gid)"
              :min="1"
              :max="item.goods.gnum"
            >
            </el-input-number>
          ...
          <div class="pro-total pro-total-in">{{(item.goods.gpride*item.buyerCart.goodsnum).toFixed(2)}}元</div>
            ...
            <el-popover placement="right">
              ...
              <div style="text-align: right; margin: 10px 0 0">
                <el-button
                  type="primary"
                  size="mini"
                  @click="deleteItem($event,item.buyerCart.id,item.goods.gid)"
                >确定</el-button>
              </div>
...
export default {
  ...
  methods: {
    ...
    checkChange (val, key, productID) {
      console.log('更新vuex中购物车商品是否勾选的状态')
      // 更新vuex中购物车商品是否勾选的状态
      console.log(val)
      console.log(key)
      this.$http
    ...
    allCheck (val) {
      let shopList = this.$store.getters.getShoppingCart
