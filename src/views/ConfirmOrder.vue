        ...
        <div class="goods-list">
          <ul>
            <li v-for="item in getCheckGoods" :key="item.id">
              <img :src="item.goods.gimgMain" />
              <span class="pro-name">{{item.goods.gname}}</span>
              <span class="pro-price">{{item.goods.gpride}}元 x {{item.buyerCart.goodsnum}}</span>
              <span class="pro-status"></span>
              <span class="pro-total">{{(item.goods.gpride * item.buyerCart.goodsnum).toFixed(2)}}元</span>
            </li>
          </ul>
        </div>
      </div>
...
<!--            </li>-->
            <li class="total">
              ...
              <span class="value">
                <span class="total-price">{{getTotalPrice.toFixed(2)}}</span>元
              </span>
            </li>
          </ul>
        </div>
...
export default {
  ...
  methods: {
    ...
    addOrder () {
      if (this.waitAddress === '') {
        this.$notify.warning('请先选择地址')
      } else {
        const waitOrder = this.getCheckGoods
        const upOrder = []
        ...
        for (let i = 0; i < waitOrder.length; i++) {
          let tempOrder = {
            bid: waitOrder[i].buyer.buyerId,
            bname: waitOrder[i].buyer.buyername,
            sname: waitOrder[i].goods.gsalerName,
            goId: waitOrder[i].goods.gid,
            boType: 0,
            baddr: allAddr,
            gname: waitOrder[i].goods.gname,
            gimg: waitOrder[i].goods.gimgMain,
            goodsnum: waitOrder[i].buyerCart.goodsnum,
            gprice: waitOrder[i].goods.gpride,
            goPrice: waitOrder[i].goods.gpride * waitOrder[i].buyerCart.goodsnum,
            sid: waitOrder[i].goods.gsalerId,
            boInfo: this.info
          }
          upOrder.push(tempOrder)
        }
        console.log(upOrder)
        this.$http
          .post('buyerOrder/addBuyerOrderList', upOrder)
          .then(res => {
            ...
            switch (res.data.code) {
              ...
              case '0000':
                for (let i = 0; i < products.length; i++) {
                  ...
                  //  删除购物车数据库
                  this.$http
                    .post('buyerCart/deleteOne', {
                      bcId: this.$store.getters.getUser.buyerId,
                      gcId: temp.buyerCart.id
                    })
                  // 删除已经结算的购物车商品
                  this.deleteShoppingCart(temp.buyerCart.id)
                }
                // 提示结算结果
        ...
        if (valid) {
          ...
          this.$http.post(`buyerAddr/addOneAddr`, {
            ...
            if (res.data.code === '0000') {
              ...
              this.form = {}
              this.confirmAddress = this.addressNum + 1
              this.reload()
              // location.reload()
            }
          })
    ...
    getAddr () {
      this.$http.get(`buyerAddr/allAddr?Bid=${this.$store.getters.getUser.buyerId}`).then(res => {
        if (res.data.code === '0000') {
          ...
          this.addressNum = res.data.data.length
        } else {
          this.address = []
          this.addressNum = 0
        }
