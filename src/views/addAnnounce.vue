...
export default {
  ...
  methods: {
    ...
    onEditorFocus () {
      ...
      // console.log(this.editor.getText().length === 0)
    }, // 获得焦点事件
    onEditorChange (val) {
      // :prop="this.editor.getText()"
      // console.log(this.editor.getText())
      // console.log(this.editor.getText() === '')
      // // console.log(this.$refs['form'].validateField('text'))
      // this.$refs['form'].validateField('text', (valid) => {
      //   // callback()
      //   return true
      // })
    }, // 内容改变事件
    onSubmit (form) {
      ...
      // console.log(this.encode(this.editor.root.innerHTML))
      this.$refs[form].validate((valid) => {
        if (valid) {
          if (this.editor.getText().length !== 1) {
            this.$http.post(`announcement/addAnnounce`, {
              borSId: this.$store.getters.getUser.buyerId,
              borSName: this.$store.getters.getUser.buyerName,
