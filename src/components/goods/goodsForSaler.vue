...
<!--          <el-input-number v-model="form.gnum" @change="handleChange" :min="1" :max="form.gnum" label="描述文字"></el-input-number>-->
        ...
        <el-form-item label="产品划线价格" :label-width="formLabelWidth" prop="glinepride">
          <el-input v-model="form.glinepride"
                    autocomplete="off"
                    placeholder="请输入产品划线价格"></el-input>
        </el-form-item>
...
export default {
  ...
  data () {
    // 这里存放数据",
    return {
      sId: this.$store.getters.getSaler.salerId,
      // 搜索值
      query: '',
      searchTF: false,
      // 分页值
  ...
  methods: {
    ...
    async searchGoods () {
      ...
      const res = await this.$http.get(`goods/searchAllGoodsForSaler?current=${this.current}&size=${this.size}&sId=${this.sId}&gName=${this.query}`)
    ...
    async getGoodsList () {
      //  获取数据
      const res = await this.$http.get(`goods/getAllGoodsForSaler?current=${this.current}&size=${this.size}&sId=${this.sId}`)
      if (res.data.data != null) {
