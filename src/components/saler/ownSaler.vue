...
export default {
  ...
  data () {
    ...
    return {
      ...
      ruleForm: {
        ...
        salerimg: '',
        salerinfo: '',
        salerId: this.$store.getters.getSaler.salerId
  ...
  methods: {
    ...
    handleAvatarSuccess (res) {
      ...
      this.ruleForm.salerimg = res.msg
    },
    ...
    submitForm (formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.$http
            .put('saler/updateOneSaler', salerForm)
            .then((res) => {
              const {
                code,
                msg
              ...
              if (code === '0000') {
                ...
                this.reload()
                const saler = {
                  salername: salerForm.salername,
                  salerId: salerForm.salerId,
                  salerImg: salerForm.salerimg
                }
                // 更新缓存
                localStorage.setItem('saler', JSON.stringify(saler))
                // 更新vuex
                this.setSaler(saler)
                // 提示成功
                this.$notify.success({
    ...
    getOneSaler () {
      let salerId = localStorage.getItem('salerId')
      this.$http.get(`saler/getOneSalerBySid/${salerId}`).then((res) => {
        const data = res.data
        this.imageUrl = data.salerimg
        this.ruleForm.salerimg = data.salerimg
        this.ruleForm.salername = data.salername
        this.ruleForm.salerinfo = data.salerinfo
        this.ruleForm.salerpass = data.salerpass
        this.ruleForm.checkPass = data.salerpass
        this.ruleForm.saleraccount = data.saleraccount
      })
    }
  }
}
